...
function WeeklymeetingWrite() {
  ...
  useEffect(async () => {
      const temporal2 = {};
      console.log(stuDBList);
      stuDBList.map(
        (element) =>
          (temporal2[element["ID"]] = {
            등교: lastweekData
              ? lastweekData["thisweekData"][element["ID"]]["등교"]
              : false,
            이번주문제사항: "",
            이번주조치계획: "",
            조치내용: "",
  ...
  useEffect(async () => {
     if (isInitialMount.current === false) {const temporal = stuDBList.map((element, i) => {
      return {
        ID: element["ID"],
        분류: element["분류"],
        이름: element["이름"],
            ...
            axios
              ...
              .then(function (result) {
                if (result.data === true) {
                  ...
                  history.push(`/Weeklymeeting/Edit/${paramDate}`);
...
export default WeeklymeetingWrite;
